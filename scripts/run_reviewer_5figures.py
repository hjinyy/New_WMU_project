...
OUT=DATA/'analysis_reviewer_7figures_v1'
...
def fig3():
    root=DATA/'analysis_basic_v1/analysis_fault_generalization_bus14_pcc_v1'
    man=pd.read_csv(root/'manifests/fault_generalization_manifest.csv')
    cases=[('Normal-like baseline',DATA/'IEEE14bus_bus14_rerun_v1/raw_csv/case_0317__BG_SSO25Hz_M03__EV_Normal__BUS_00.csv',0.3,0.36,'#444444'),('ThreePhase R=0.1Ω',root/'raw_csv/ieee14_case_0514__FT_ThreePhase__BUS_14__R_0p1__ANG_000__BG_NoSSO.csv',0.3,0.36,'#D55E00'),('ThreePhase R=10Ω',root/'raw_csv/ieee14_case_0532__FT_ThreePhase__BUS_14__R_10__ANG_000__BG_NoSSO.csv',0.3,0.36,'#E69F00'),('SLG R=10Ω',root/'raw_csv/ieee14_case_0451__FT_SLG__BUS_14__R_10__ANG_000__BG_NoSSO.csv',0.3,0.36,'#0072B2')]
...
def fig4():
    cm14=pd.read_csv(DATA/'analysis_basic_v1/results_basic_v1/ieee14_ExtraTrees_event_confusion_matrix.csv',index_col=0)
    cm30=pd.read_csv(DATA/'analysis_basic_v1/results_basic_v1/ieee30_ExtraTrees_event_confusion_matrix.csv',index_col=0)
    b14=pd.read_csv(DATA/'analysis_basic_v1/results_basic_v1/full_wmu_baseline_ieee14.csv')
    b30=pd.read_csv(DATA/'analysis_basic_v1/results_basic_v1/full_wmu_baseline_ieee30.csv')
...
def fig5():
    ...
    known14=float(pd.read_csv(DATA/'analysis_basic_v1/results_basic_v1/full_wmu_baseline_ieee14.csv').query("Model=='ExtraTrees' and Task=='fault_localization_full_wmu'").iloc[0].ExactBusAccuracy)
    known30=float(pd.read_csv(DATA/'analysis_basic_v1/results_basic_v1/full_wmu_baseline_ieee30.csv').query("Model=='ExtraTrees' and Task=='fault_localization_full_wmu'").iloc[0].ExactBusAccuracy)
    u14=pd.read_csv(DATA/'analysis_basic_v1/analysis_fault_generalization_bus14_pcc_v1/results/unseen_resistance_results.csv')
    ...
    u30=pd.read_csv(DATA/'analysis_basic_v1/analysis_fault_generalization_v1/results/unseen_resistance_results.csv')
...
def fig7_feature_boxplots():
    ...
    for row,(network,path) in enumerate([('IEEE14',DIAG/'reviewer_raw_physical_features_from_current_waveforms.csv.gz'),('IEEE30',DIAG/'reviewer_raw_physical_features_from_current_waveforms.csv.gz')]):
        df=pd.read_csv(path)
        df=df[df.NetworkID==network.lower()].copy()
...
def main():
    ...
    (OUT/'README.md').write_text('# Reviewer seven-figure set\n\nSeven figures for methodology and experimental-results explanation. Generated from existing validated WMU raw/result files; no new simulation or ML training was run for this figure export.\n',encoding='utf-8')
    print(f'Wrote {OUT}')
