function run_fault_generalization_v1(mode)
% Fault-parameter generalization pilot for IEEE14/IEEE30 WMU datasets.
% Writes only under /home/hy/문서/WMU_project/analysis_basic_v1/analysis_fault_generalization_v1.
if nargin < 1 || isempty(mode), mode = "all"; end
mode = string(mode);
root = '/home/hy/문서/WMU_project/analysis_basic_v1/analysis_fault_generalization_v1';
ensure_dirs(root);
...
assert(sum(T.NetworkID=="ieee30") == 5*4*3*3*3, 'IEEE30 case count mismatch');
assert(height(T) == 1080, 'total case count mismatch');
end
...
ieee30_model = getenv('WMU30_MODEL_OVERRIDE');
if isempty(ieee30_model)
    ieee30_model = '/home/hy/문서/matlab/Thirtybussys_WMU_IBR_auto.slx';
end
configs(2) = struct('network_id','ieee30','num_buses',30,'model',ieee30_model,'fault_buses',[1 6 10 24 30],'backgrounds',bgs);
end
